...
namespace GuiCookie.Core.Elements
{
    public class SliderBar : ProgressBar, IClickable
    {
        ...
        private readonly Signal onValueChanged = new();
        ...
        protected override void Update(TimeSpan elapsedTime, TimeSpan totalTime)
        {
            ...
            if (mouseHandler.IsClickDragged)
            {
                ...
                float mousePos = LayoutDirection == Direction.Horizontal
                    ? Math.Clamp(mouseHandler.RelativeMousePosition.X, halfHandleSize, Bounds.TotalSize.X - halfHandleSize) - halfHandleSize
                    : Math.Clamp(mouseHandler.RelativeMousePosition.Y, halfHandleSize, Bounds.TotalSize.Y - halfHandleSize) - halfHandleSize;
